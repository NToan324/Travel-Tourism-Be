...
class KnowledgeService {
  async update(
    ...
    if (payload.file_name !== undefined)
      knowledge.file_name = payload.file_name;
    if (payload.name !== undefined) knowledge.name = payload.name;
    if (payload.mime_type !== undefined)
      knowledge.mime_type = payload.mime_type;
    ...
    if (
      ...
      knowledge.metadata = {
        topic:
          payload.topic !== undefined ?
            payload.topic
          : knowledge.metadata?.topic || "",
        location:
          payload.location !== undefined ?
            payload.location
          : knowledge.metadata?.location || "",
        source:
          payload.source !== undefined ?
            payload.source
  ...
  async synchronize(id: string) {
    ...
    try {
      console.log(
        `[Sync] Starting stream for: ${knowledge.name} (${knowledge.url})`
      );
  ...
  async desynchronize(id: string) {
    ...
    try {
      ...
      throw new InternalServerError(
        `Failed to remove from AI: ${error.message}`
      );
    }
  }
  ...
  async delete(id: string) {
    ...
    if (knowledge.status === "synced" || knowledge.status === "error") {
      try {
        ...
        console.warn(
          "[Delete] Warning: Could not delete from Python (might already be deleted)",
          error
        );
      }
    }
    ...
    try {
      ...
      console.warn(
        "[Delete] Warning: Could not delete file from Cloudinary (might already be deleted)",
        error
      );
    }
